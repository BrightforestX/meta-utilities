...
- **cli/** — Opt-in terminal interfaces. The main one today is `cli/oteemo-assistant/` (rich Ink TUI with `@assistant-ui/react-ink` primitives + a new `--headless` / `--command` one-shot path for scripts/CI that bypasses the UI and uses McpManager+parseIntent directly for the same intents: run oteemo, ontology ops, health, px pulls). See `cli/README.md` (headless section) for usage. The primary reproducible/headless surfaces are the Python CLIs under the mcp-servers (scenario-research with --target modal support, meta-batch, etc.). All are flag-driven, non-tty safe, and pipe/redirect friendly.
