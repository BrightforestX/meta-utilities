...
- **cli/** — Opt-in terminal interfaces. The main one today is `cli/oteemo-assistant/` (rich Ink TUI with `@assistant-ui/react-ink` primitives, persistent status bar showing MODE/MODEL/PX/running state, oteemo simulation, live px context, and ontology recall commands). See `cli/README.md` for current usage. The primary reproducible surfaces are the Python CLIs under the mcp-servers (especially `scenario-research`).
